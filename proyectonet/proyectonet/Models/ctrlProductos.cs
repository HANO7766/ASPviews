...
internal class ctrlProductos
{
    public List<object> consulta(String dato)
    {
        ...
        {
            sql = "SELECT id, nombre, descripcion, precio FROM productos WHERE id LIKE '%" + dato + "%' OR nombre LIKE '%" + dato + "%' OR descripcion LIKE '%" + dato + "%' OR precio LIKE '%" + dato + "%' ORDER BY nombre ASC";
        }
        try
        {
